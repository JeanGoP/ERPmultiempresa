...
2. Transferir de forma segura el `.env` privado y dejarlo junto a `NexoERP.Api.exe` en el servidor. Debe contener `ConnectionStrings__NexoErp` apuntando a una instancia compartida de SQL Server; las variables del sistema operativo tienen prioridad y producción rechaza explícitamente LocalDB.
...
No confirmes archivos `.env` en Git ni los incluyas en el paquete `publish/`; transfiérelos por separado. Tampoco publiques contraseñas, respaldos ni bases locales.

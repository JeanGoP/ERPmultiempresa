...
for(const tag of ['TaxTotal','WithholdingTaxTotal']){
  ...
  assert.equal(mixed.totals.retentions,200);assert.equal(mixed.items[0].retention,200);assert.equal(mixed.taxes.length,0);
  const lineOnly=parse('',retention(0.5,50,tag)+retention(2.5,250,tag));
  assert.equal(lineOnly.totals.retentions,250);assert.equal(lineOnly.items[0].retention,250);
  const duplicated=parse(retention(2,200,tag),retention(2,200,tag));assert.equal(duplicated.items[0].retention,200);
}
assert.equal(parse(retention(null,120)).totals.retentions,0);
...
console.log('QA retenciones: umbral 2%, fuentes XML, total editable, distribución, payload y bloqueo tras guardar correctos.');

...
(async()=>{
  ...
  try{
    ...
    for(const file of ['vendor/jspdf.umd.min.js','app.js','accounts-payable.js','supplier-payment-report.js','payment-report-excel.js'])await page.addScriptTag({path:path.join(root,'public',file)});
    await page.evaluate(template=>{window.excelTemplate=template;window.fetch=async url=>{if(url==='assets/payment-report-template.json')return {ok:true,json:async()=>template};throw new Error('Solicitud inesperada: '+url);};},JSON.parse(fs.readFileSync(path.join(root,'public/assets/payment-report-template.json'),'utf8')));
    await page.evaluate(()=>{
    ...
    await page.locator('[data-report-download="xlsx"]').click();
    ...
    assert((await page.evaluate(()=>saved.name)).endsWith('.xlsx'));
    const xlsx=await page.evaluate(async()=>Array.from(new Uint8Array(await saved.content.arrayBuffer())));
    fs.writeFileSync(path.join(output,'relacion.xlsx'),Buffer.from(xlsx));
    assert.strictEqual(Buffer.from(xlsx).readUInt32LE(),0x04034b50);
    const noPayments=await page.evaluate(async()=>{const copy=structuredClone(fixture);copy.movimientos=copy.movimientos.filter(x=>x.tipoMovimiento==='FACTURA');copy.facturas[0].saldoPendiente=1166200;const blob=await buildPaymentReportXlsx(preparePaymentReport(copy,[1],'COP'),excelTemplate);return Array.from(new Uint8Array(await blob.arrayBuffer()));});
    fs.writeFileSync(path.join(output,'sin-pagos.xlsx'),Buffer.from(noPayments));
    const hostile=await page.evaluate(async()=>{const report=preparePaymentReport(fixture,[1],'COP');report.rows[0].numeroDocumento='=HYPERLINK("https://example.invalid")';report.empresa='Nombre & <etiqueta> "literal"';const blob=await buildPaymentReportXlsx(report,excelTemplate);return Array.from(new Uint8Array(await blob.arrayBuffer()));});
    fs.writeFileSync(path.join(output,'texto-seguro.xlsx'),Buffer.from(hostile));
    ...
    console.log('Relación: selección, monedas, desglose, notas, datos faltantes, cambio de empresa, descarga XLSX real y PDF verificados.');

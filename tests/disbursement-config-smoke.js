...
(async()=>{
 await context.configure({});assert.equal(elements['[name="account"]'].value,'111005');assert.ok(elements['[data-back]'].removed);
 await elements['[data-config-form]'].onsubmit({preventDefault(){},stopPropagation(){}});
 assert.equal(writes,1);assert.match(elements['[data-notice]'].textContent,/guardada/);
 assert.ok(requests.every(x=>x.startsWith('/api/v1/companies/1/')));
 root.isConnected=false;await elements['[data-config-form]'].onsubmit({preventDefault(){},stopPropagation(){}});assert.equal(writes,1,'Pantalla retirada no guarda configuración');

...
INSERT inv.Marca(EmpresaId,Nombre) VALUES(@A,'MARCA A'),(@A,'MARCA B'),(@B,'MARCA B');
UPDATE c SET MarcaId=m.MarcaId FROM inv.CatalogoMarcaDescripcion c JOIN inv.Marca m ON m.EmpresaId=c.EmpresaId AND m.Nombre=c.Marca COLLATE Latin1_General_100_CI_AI WHERE c.EmpresaId IN(@A,@B);
...
IF COALESCE((SELECT Marca FROM inv.fn_MarcaPorReferenciaDescripcion(@A,'1',N'Abreviatura distinta')),'')<>'MARCA A' THROW 51620,'No reconoció la referencia exacta.',1;
IF COALESCE((SELECT Marca FROM inv.fn_MarcaPorReferenciaDescripcion(@A,'NO-EXISTE',N'Nevera frío 25')),'')<>'MARCA A' THROW 51621,'No usó descripción de respaldo.',1;
IF (SELECT Marca FROM inv.fn_MarcaPorReferenciaDescripcion(@A,'5',N'Nevera frío 25')) IS NOT NULL THROW 51622,'Una referencia pendiente no debe usar otra marca por descripción.',1;
UPDATE inv.Marca SET Activa=0 WHERE EmpresaId=@A AND Nombre='MARCA A';
IF (SELECT Marca FROM inv.fn_MarcaPorDescripcion(@A,N'Nevera frío 25')) IS NOT NULL THROW 51617,'No debe reconocer marcas inactivas.',1;
UPDATE inv.Marca SET Activa=1,Nombre='MARCA RENOMBRADA' WHERE EmpresaId=@A AND Nombre='MARCA A';
IF COALESCE((SELECT Marca FROM inv.fn_MarcaPorDescripcion(@A,N'Nevera frío 25')),'')<>'MARCA RENOMBRADA' THROW 51618,'No refleja el nombre del maestro.',1;
...
IF EXISTS(SELECT 1 FROM inv.Marca WHERE EmpresaId=@B) THROW 51619,'RLS no aísla las marcas.',1;
...
IF (SELECT Marca FROM inv.fn_MarcaPorReferenciaDescripcion(@B,'1',N'Abreviatura')) IS NOT NULL THROW 51623,'La referencia filtró información de otra empresa.',1;

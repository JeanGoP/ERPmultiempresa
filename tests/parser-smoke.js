...
assert.strictEqual(variants.totals.retentions, 125);
...
for(const rate of [0.5,0.99,1.2,1.99,2,2.5]){
  ...
  assert.strictEqual(parsed.totals.retentions,125);
  assert.strictEqual(parsed.items.reduce((sum,item)=>sum+item.retention,0),125);
  assert.strictEqual(parsed.totals.payable,11775,'La lectura conserva el total informado por el proveedor');
}
assert.strictEqual(parserContext.parseInvoiceForTest(variantsFixture.replace('<cbc:Percent>1.25</cbc:Percent>','')).totals.retentions,125,'Conservar retenciones ordinarias sin porcentaje');
assert.strictEqual(parserContext.parseInvoiceForTest(variantsFixture.replace('ReteRenta','Autorretención de Renta').replace('</Invoice>','<CustomField Name="TotalRetenciones" Value="125"/></Invoice>')).totals.retentions,0,'No reintroducir autorretenciones mediante campos personalizados');

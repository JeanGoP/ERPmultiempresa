...
"@
    ...
    if($health.status -ne 'ok' -or $health.migrations -ne 42 -or $health.release -ne '2026.09.01.2' -or $health.databaseMode -ne 'localdb' -or [string]::IsNullOrWhiteSpace($health.databaseFingerprint)){ throw 'La salud de la API no reportó versión, conexión y migraciones esperadas.' }
    ...
    $supplierFromXml=Invoke-RestMethod -Uri "$baseUrl/api/v1/companies/$companyId/master-data/suppliers/from-xml" -Headers $operatorHeaders -Method Post -ContentType 'application/json' -Body (@{tipoIdentificacion='NIT';numeroIdentificacion='901111222';digitoVerificacion='3';razonSocial='Proveedor cargado al analizar XML';nombreComercial='Proveedor XML';codigoResponsabilidadFiscal='O-13';regimenFiscalCodigo='01';regimenFiscalNombre='IVA';direccion='Calle XML 10';ciudadCodigo='11001';ciudad='Bogota';departamentoCodigo='11';departamento='Bogota D.C.';codigoPostal='110111';paisCodigo='CO';pais='Colombia';contactoNombre='Contacto XML';telefono='6015550101';correo='xml@proveedor.local';sitioWeb=$null;datosXmlJson='{"Party.CompanyID":[{"value":"901111222"}]}' }|ConvertTo-Json)
    $suppliersFromXml=Invoke-RestMethod -Uri "$baseUrl/api/v1/companies/$companyId/master-data/suppliers" -Headers $adminHeaders -Method Get
    $persistedFromXml=$null
    foreach($supplierRow in $suppliersFromXml){if($supplierRow.numeroIdentificacion -eq '901111222'){$persistedFromXml=$supplierRow;break}}
    $supplierRowCount=[int]((& sqlcmd -S $Instance -E -h -1 -W -d $databaseName -Q "SET NOCOUNT ON; EXEC sys.sp_set_session_context @key=N'BypassRls',@value=1; SELECT COUNT(*) FROM ter.Tercero WHERE EmpresaId=$companyId AND NumeroIdentificacion='901111222' AND EsProveedor=1 AND Correo='xml@proveedor.local';")|Select-Object -First 1).Trim()
    if($supplierFromXml.id -le 0 -or $supplierRowCount -ne 1 -or $persistedFromXml.razonSocial -ne 'Proveedor cargado al analizar XML' -or $persistedFromXml.correo -ne 'xml@proveedor.local'){ throw "Analizar el XML no persistió el proveedor en ter.Tercero. guardado=$(ConvertTo-Json $supplierFromXml -Compress) filasSql=$supplierRowCount proveedores=$(ConvertTo-Json $suppliersFromXml -Compress)" }
    $deleteSupplier=Invoke-WebRequest -UseBasicParsing -Uri "$baseUrl/api/v1/companies/$companyId/master-data/suppliers/$($supplierFromXml.id)" -Headers $adminHeaders -Method Delete
    $supplierRowCountAfterDelete=[int]((& sqlcmd -S $Instance -E -h -1 -W -d $databaseName -Q "SET NOCOUNT ON; EXEC sys.sp_set_session_context @key=N'BypassRls',@value=1; SELECT COUNT(*) FROM ter.Tercero WHERE EmpresaId=$companyId AND TerceroId=$($supplierFromXml.id);")|Select-Object -First 1).Trim()
    if($deleteSupplier.StatusCode -ne 204 -or $supplierRowCountAfterDelete -ne 0){ throw 'El maestro no eliminó de ter.Tercero un proveedor sin historial.' }
    ...
    foreach($supplierRow in $suppliersAfterXml){if($supplierRow.numeroIdentificacion -eq '890301886'){$supplierAfterXml=$supplierRow;break}}
    ...
    $supplierEdited=$null
    foreach($supplierRow in $suppliersEdited){if($supplierRow.terceroId -eq $supplierAfterXml.terceroId){$supplierEdited=$supplierRow;break}}

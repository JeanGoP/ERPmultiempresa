...
function renderAccountsPayable(){
  const data=state.accountsPayable||{resumen:{facturas:0,facturasVencidas:0,saldoTotal:0,saldoVencido:0,saldoPorVencer:0},documentos:[]};const summary=data.resumen;
  const cards=[['Facturas abiertas',summary.facturas],['Saldo total',manualCurrency(summary.saldoTotal)],['Saldo vencido',manualCurrency(summary.saldoVencido)],['Por vencer',manualCurrency(summary.saldoPorVencer)],['Facturas vencidas',summary.facturasVencidas]];
  elements.accountsPayableStats.replaceChildren();cards.forEach(([label,value])=>{const card=document.createElement('article');const strong=document.createElement('strong');strong.textContent=value;const span=document.createElement('span');span.textContent=label;card.append(strong,span);elements.accountsPayableStats.append(card);});
  const table=document.createElement('table');const head=document.createElement('thead');head.innerHTML='<tr><th>Proveedor</th><th>Factura</th><th>Contabilizada</th><th>Vencimiento</th><th>Valor original</th><th>Saldo</th><th>Edad</th><th>Estado</th><th></th></tr>';const body=document.createElement('tbody');
  data.documentos.forEach(item=>{const row=document.createElement('tr');const supplier=document.createElement('td');supplier.innerHTML=`<strong>${item.proveedor}</strong><small>${item.proveedorIdentificacion}</small>`;const documentCell=document.createElement('td');documentCell.innerHTML=`<strong>${item.numeroDocumento}</strong><small>${item.tipoDocumento} · ${item.condicionPago==='CREDITO'?'Crédito':'Contado'}</small>`;const recognized=document.createElement('td');recognized.textContent=item.fechaReconocimiento;const due=document.createElement('td');due.textContent=item.fechaVencimiento;const original=document.createElement('td');original.textContent=manualCurrency(item.valorOriginal);const balance=document.createElement('td');balance.className='accounts-payable-balance';balance.textContent=manualCurrency(item.saldoPendiente);const age=document.createElement('td');age.textContent=item.rangoEdad;const status=document.createElement('td');const badge=document.createElement('span');badge.className=`saved-purchase-badge state-${item.estado.toLowerCase()}`;badge.textContent=item.estado;status.append(badge);const action=document.createElement('td');const open=document.createElement('button');open.type='button';open.className='button secondary';open.dataset.payableDocumentId=item.documentoProveedorId;open.textContent='Ver factura';action.append(open);row.append(supplier,documentCell,recognized,due,original,balance,age,status,action);body.append(row);});
  if(!data.documentos.length){const row=document.createElement('tr');const cell=document.createElement('td');cell.colSpan=9;cell.className='empty';cell.textContent='No hay cuentas por pagar para los filtros seleccionados. Las facturas aparecen aquí únicamente después de quedar contabilizadas.';row.append(cell);body.append(row);}table.append(head,body);elements.accountsPayableTable.replaceChildren(table);elements.accountsPayableStatus.textContent=`${data.documentos.length} factura(s) consultada(s)`;
}

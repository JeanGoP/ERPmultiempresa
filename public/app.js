...
const elements = {
  ...
  accountsPayableModule:$('#accountsPayableModule'),accountsPayableNav:$('#accountsPayableNav'),accountsPayableStats:$('#accountsPayableStats'),accountsPayableStatus:$('#accountsPayableStatus'),accountsPayableSearch:$('#accountsPayableSearch'),accountsPayableSupplier:$('#accountsPayableSupplier'),accountsPayableState:$('#accountsPayableState'),accountsPayableFrom:$('#accountsPayableFrom'),accountsPayableTo:$('#accountsPayableTo'),accountsPayableTable:$('#accountsPayableTable'),accountsPayableNotice:$('#accountsPayableNotice'),refreshAccountsPayable:$('#refreshAccountsPayable'),
...
async function apiRequest(path, options = {}) {
  // All receipt posting entry points share the same warehouse confirmation.
  if(options.method==='POST'&&/\/receipts\/\d+\/post$/.test(path)){
    const body=JSON.parse(options.body||'{}');
    const distribution=await chooseReceiptWarehouses(path.replace(/\/post$/,'/distribution'));
    body.bodegas=distribution.bodegas;body.sucursalId=distribution.sucursalId;
...
function renderErpSession() {
  ...
  elements.companySwitcher.disabled=!session.superAdmin;
  elements.companySwitcher.setAttribute('aria-label',session.superAdmin?'Cambiar empresa':'Empresa asignada');
...
function canUseWarehouseHistory() { return hasPermission('SEGURIDAD.PERMISOS.ADMINISTRAR'); }
...
function isInventoryViewAllowed(view) {
  ...
  if (view === 'receipt-history' || view === 'receipt-issues') return canUseWarehouseHistory();
  if (view === 'operations') return canUseInventoryOperations();
  return canUseInventoryReports();
}
function firstAllowedInventoryView() {
  return ['receiving', 'receipt-issues', 'receipt-history', 'stock', 'kardex', 'serials', 'expiry', 'operations'].find(isInventoryViewAllowed) || null;
}
function hideWorkspaces() {
  ...
  elements.accountsPayableModule.hidden = true;
...
function isCurrentWorkspaceAllowed() {
  if(document.querySelector('#zeusModule')?.hidden===false) return hasAnyPermission(['SEGURIDAD.PERMISOS.ADMINISTRAR','COMPRAS.RECEPCION.CONTABILIZAR']);
  ...
  if (!elements.accountsPayableModule.hidden) return canUseAccountsPayable();
...
async function initializeErpUi() {
  ...
  if (savedSession?.email && savedSession?.company && savedSession.api && apiToken()) {
    elements.loginView.hidden=true;elements.erpShell.hidden=true;
    try {
      const identity=await apiRequest('/api/v1/auth/me');
      const companies=await apiRequest('/api/v1/companies');
      state.pendingEmail=savedSession.email;state.pendingName=savedSession.name;state.pendingSuperAdmin=identity.esSuperAdministrador;
      const company=companies.find(c=>String(c.empresaId)===String(identity.esSuperAdministrador?savedSession.company.id:identity.empresaId));
      renderCompanyOptions(companies);
      if(company){state.erpSession=null;selectCompany([...$('#companyOptions').children].find(x=>x.dataset.companyId===String(company.empresaId)));}
      else if(identity.esSuperAdministrador){elements.loginView.hidden=false;configureSuperAdminCompanyPanel(true,companies.length>0);openErpDialog(elements.companyDialog);}
      else throw new Error('No tienes una empresa activa asignada.');
    } catch(error){leaveErp();elements.loginError.textContent=error.message;elements.loginError.hidden=false;}
  }
...
async function loadApiCompanyContext() {
  ...
  try {
    ...
    state.apiContext={ warehouses,periods,accountingPeriods,accounts,permissions,permissionCodes:new Set(permissions.map(permissionCode)),masterData:{
      ...
      warehouses:warehouses.map(x=>({id:x.bodegaId,code:x.codigo,name:x.nombre,locations:x.usaUbicaciones,transit:x.esTransito,sucursalId:x.sucursalId,active:true})),
...
function renderManualReferenceOptions() {
  ...
  elements.manualPeriodField.hidden=true; elements.manualPeriod.required=false;
}
...
function renderMasterView() {
  ...
  if(state.masterView==='warehouses'){
    ...
    const table=buildDataTable(['Código','Nombre','Sucursal','Ubicaciones','Tránsito','Estado','Acciones'],warehouses.map(x=>[x.code,x.name,(data.branches||[]).find(b=>b.id===x.sucursalId)?.nombre||'Pendiente de asignar',x.locations?'Sí':'No',x.transit?'Sí':'No',activeLabel(x.active),'']));
    table.querySelectorAll('tbody tr').forEach((row,index)=>{const warehouse=warehouses[index];if(!warehouse)return;const edit=document.createElement('button');edit.type='button';edit.className='button secondary';edit.textContent='Editar';edit.onclick=()=>openMasterForm(warehouse);row.lastElementChild.append(edit);if(hasPermission('SEGURIDAD.PERMISOS.ADMINISTRAR'))row.lastElementChild.append(warehouseAccountsButton(warehouse));});
...
async function loadSecurityData(){
  ...
  const company=state.erpSession.company.id,token=apiToken();
  ...
  if(company!==state.erpSession?.company?.id||token!==apiToken())throw new Error('Cambió la sesión o empresa durante la consulta. Actualiza la pantalla.');
...
async function openSecurityUser(user=null){
  state.securityEditingUserId=user?.usuarioId||null;elements.securityUserForm.reset();elements.securityUserError.hidden=true;elements.securityUserIdentity.hidden=Boolean(user);elements.securityUserIdentity.querySelectorAll('input').forEach(input=>input.disabled=Boolean(user));$('#securityUserDialogTitle').textContent=user?'Editar acceso':'Nuevo usuario';$('#securityUserDialogSubtitle').textContent=user?`${user.nombreCompleto} · ${user.correo}`:'Asigna su empresa y sus roles. Ingresará directamente a esa empresa.';elements.securityUserForm.elements.accesoActivo.checked=user?.accesoActivo??true;fillSecurityRoles(user?.roles.map(x=>x.rolId)||[]);
  const select=$('#securityUserCompany'),company=state.erpSession.company;
  select.replaceChildren(new Option(company.name,company.id));select.value=company.id;select.disabled=true;
  openErpDialog(elements.securityUserDialog);
  if(!user&&state.erpSession.superAdmin){try{const companies=await apiRequest('/api/v1/companies');if(!elements.securityUserDialog.open||state.securityEditingUserId||state.erpSession?.company?.id!==company.id)return;select.replaceChildren(...companies.map(c=>new Option(c.razonSocial,c.empresaId)));select.value=company.id;select.disabled=false;}catch(error){elements.securityUserError.textContent=error.message;elements.securityUserError.hidden=false;}}
}
...
async function saveSecurityUser(event){
  ...
  const submit=elements.securityUserForm.querySelector('[type="submit"]');submit.disabled=true;
  try{if(!roleIds.length)throw new Error('Selecciona al menos un rol.');const target=edit?state.erpSession.company.id:$('#securityUserCompany').value;if(!target)throw new Error('Selecciona la empresa del usuario.');if(!state.erpSession.superAdmin&&String(target)!==String(state.erpSession.company.id))throw new Error('Solo puedes crear usuarios de tu empresa.');await apiRequest(`/api/v1/companies/${target}/security/users${edit?`/${state.securityEditingUserId}`:''}`,{method:edit?'PUT':'POST',body:JSON.stringify(payload)});const companyName=$('#securityUserCompany').selectedOptions[0].textContent;closeErpDialog(elements.securityUserDialog);await refreshSecurity();elements.securityNotice.textContent=`Usuario guardado en ${companyName}. Su empresa no se escoge al iniciar sesión.`;elements.securityNotice.hidden=false;}catch(error){elements.securityUserError.textContent=error.message;elements.securityUserError.hidden=false;}finally{submit.disabled=false;}
}
...
function renderSavedPurchaseDetail(){
  ...
  if(inventoryLines.length){
    ...
    const post=document.createElement('button');post.type='button';post.className='button primary large';post.textContent=entry.recepcionMercanciaId?'Contabilizar entrada':'Preparar y contabilizar entrada';post.disabled=!entry.recepcionMercanciaId&&(!warehouse.value||!period.value);const validate=()=>{const missing=!period.value||(!entry.recepcionMercanciaId&&!warehouse.value);post.disabled=missing;if(prepareOnly)prepareOnly.disabled=missing;};warehouse.addEventListener('change',validate);period.addEventListener('change',validate);
    bindReceiptAccountingDate(date,period,panel,validate);
    post.addEventListener('click',async()=>{if(!window.confirm(`Se afectarán las existencias y el Kardex con la factura ${entry.numeroDocumento}. Cuando todos sus procesos queden contabilizados también se generará la cuenta por pagar al proveedor. ¿Deseas continuar?`))return;try{if(!receiptPeriodForDate(date.value))throw new Error('Abre el período de la fecha contable antes de contabilizar.');post.disabled=true;post.textContent='Contabilizando…';let receiptId=entry.recepcionMercanciaId;if(!receiptId){const prepared=await prepareSupplierDocumentForReceipt(entry.documentoProveedorId,{hasInventory:true,hasServices:serviceLines.length>0,bodegaId:warehouse.value,periodoInventarioId:period.value,fechaContable:date.value,numeroDocumento:entry.numeroDocumento});receiptId=prepared.recepcionMercanciaId;}await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/receipts/${receiptId}/post`,{method:'POST',body:JSON.stringify({fechaContable:date.value,correlationId:crypto.randomUUID?crypto.randomUUID():null})});await Promise.all([refreshSavedPurchases(),loadApiCompanyContext()]);await openSavedPurchase(entry.documentoProveedorId);showSuccess('Entrada contabilizada. Si la factura completó todos sus procesos, ya aparece en Cuentas por pagar.');}catch(error){showError(`No fue posible contabilizar la entrada. ${error.message}`);post.disabled=false;post.textContent=entry.recepcionMercanciaId?'Contabilizar entrada':'Preparar y contabilizar entrada';}});actions.append(post);
  }
...
function showSavedPurchases(){
  ...
  elements.purchaseModule.hidden=true;elements.masterDataModule.hidden=true;elements.savedPurchasesModule.hidden=false;elements.accountsPayableModule.hidden=true;elements.inventoryModule.hidden=true;elements.advancedControlsModule.hidden=true;elements.securityModule.hidden=true;elements.accountsPayableNav.classList.remove('active');elements.inventoryNav.classList.remove('active');elements.controlsNav.classList.remove('active');elements.securityAdminNav.classList.remove('active');document.querySelector('.breadcrumb span').textContent='Compras';elements.breadcrumbCurrent.textContent='Entradas guardadas';document.querySelectorAll('[data-registration-mode],[data-master-view],[data-inventory-view]').forEach(x=>x.classList.remove('active'));elements.savedPurchasesNav.classList.add('active');elements.savedPurchaseDetail.hidden=true;if(!state.apiContext&&state.runtimeMode==='api'&&state.erpSession?.api)void loadApiCompanyContext();void refreshSavedPurchases();window.scrollTo({top:0,behavior:'smooth'});
}

function populateAccountsPayableSuppliers(){
  const selected=elements.accountsPayableSupplier.value;elements.accountsPayableSupplier.replaceChildren(new Option('Todos los proveedores',''));
  (state.apiContext?.masterData?.suppliers||[]).filter(x=>x.active!==false).sort((a,b)=>a.name.localeCompare(b.name,'es')).forEach(x=>elements.accountsPayableSupplier.add(new Option(`${x.name} · ${x.identification}`,x.id)));
...
const warehouseIssueDemo=[
  {recepcionMercanciaRevisionUnidadId:9001,recepcionMercanciaId:501,recepcionNumero:'REC-FV-98251',documentoProveedorId:301,tipoDocumento:'FACTURA',numeroDocumento:'FV-98251',proveedor:'Motos Andinas SAS',fechaDocumento:'2026-08-28',fechaContable:'2026-08-28',bodega:'Bodega principal',recepcionMercanciaUnidadId:7001,numeroLinea:1,codigoArticulo:'357683',descripcion:'Motocicleta XR190L 2.0',numeroUnidad:2,serial:'XR190-000185',motor:'KD05E-9102842',chasis:'9FMKD0506TB001842',vin:'9FMKD0506TB001842',color:'Rojo',modelo:'2027',estadoFisico:'RECIBIDA_NOVEDAD',observacion:'Golpe visible en el guardabarros delantero derecho.',revisadoPor:'Auxiliar de bodega',revisadoEnUtc:'2026-08-28T16:42:00Z'},
  {recepcionMercanciaRevisionUnidadId:9002,recepcionMercanciaId:502,recepcionNumero:'REC-FV-98263',documentoProveedorId:302,tipoDocumento:'FACTURA',numeroDocumento:'FV-98263',proveedor:'Distribuidora Nacional de Motos',fechaDocumento:'2026-08-30',fechaContable:'2026-08-30',bodega:'Bodega principal',recepcionMercanciaUnidadId:7002,numeroLinea:2,codigoArticulo:'MOT-NAVI',descripcion:'Motocicleta Navi',numeroUnidad:1,serial:'NAVI-008241',motor:'JK07E-208241',chasis:'9FMJK0702TB008241',vin:'9FMJK0702TB008241',color:'Negro',modelo:'2027',estadoFisico:'NO_RECIBIDA',observacion:'La unidad figura en la factura pero no llegó en el vehículo transportador.',revisadoPor:'Auxiliar de bodega',revisadoEnUtc:'2026-08-30T19:15:00Z'}
];
...
function renderWarehouseHistoryStats(rows){const totals=rows.reduce((all,x)=>({units:all.units+x.unidadesSerializadas,reviewed:all.reviewed+x.revisadas,posted:all.posted+(x.estado==='CONTABILIZADA'?1:0),issues:all.issues+x.recibidasConNovedad+x.noRecibidas}),{units:0,reviewed:0,posted:0,issues:0});const cards=[['Recepciones registradas',rows.length],['Contabilizadas',totals.posted],['Checks físicos',totals.reviewed],['Novedades / no recibidas',totals.issues]];elements.inventoryStats.replaceChildren();cards.forEach(([label,value])=>{const card=document.createElement('article');const strong=document.createElement('strong');strong.textContent=value;const span=document.createElement('span');span.textContent=label;card.append(strong,span);elements.inventoryStats.append(card);});}
function renderWarehouseReceiptHistory(rows){const table=document.createElement('table');table.className='warehouse-history-table';const head=document.createElement('thead');head.innerHTML='<tr><th>Recepción</th><th>Documento</th><th>Proveedor</th><th>Bodega</th><th>Fecha</th><th>Motos</th><th>Revisión física</th><th>Novedades</th><th></th></tr>';const body=document.createElement('tbody');rows.forEach(entry=>{const row=document.createElement('tr');const receipt=document.createElement('td');receipt.className='saved-document-identity';const strong=document.createElement('strong');strong.textContent=entry.numero;const small=document.createElement('small');small.textContent=entry.estado;receipt.append(strong,small);const documentCell=document.createElement('td');documentCell.textContent=`${entry.tipoDocumento} ${entry.numeroDocumento}`;const supplier=document.createElement('td');supplier.textContent=entry.proveedor;const warehouse=document.createElement('td');warehouse.textContent=entry.bodega;const date=document.createElement('td');date.textContent=entry.fechaContable;const units=document.createElement('td');units.textContent=entry.unidadesSerializadas;const review=document.createElement('td');review.textContent=`${entry.revisadas}/${entry.unidadesSerializadas} revisadas`;const issues=document.createElement('td');issues.textContent=`${entry.recibidasConNovedad} novedad · ${entry.noRecibidas} no recibida`;const action=document.createElement('td');const open=document.createElement('button');open.type='button';open.className='button secondary';open.dataset.warehouseHistoryId=entry.recepcionMercanciaId;open.textContent='Ver informe';action.append(open);row.append(receipt,documentCell,supplier,warehouse,date,units,review,issues,action);body.append(row);});if(!rows.length){const row=document.createElement('tr');const cell=document.createElement('td');cell.colSpan=9;cell.className='empty';cell.textContent='No hay recepciones registradas para estos filtros.';row.append(cell);body.append(row);}table.append(head,body);elements.inventoryTable.replaceChildren(table);renderWarehouseHistoryStats(rows);elements.inventoryStatus.textContent=`${rows.length} recepción(es) en historial`;}
async function refreshWarehouseReceiptHistory(){elements.inventoryStatus.textContent='Consultando historial…';elements.inventoryNotice.hidden=true;try{if(!canUseWarehouseHistory())throw new Error('El historial de recepciones está disponible solo para administradores.');if(state.runtimeMode!=='api'||!state.erpSession?.api)throw new Error('El historial requiere el modo API ERP.');const params=new URLSearchParams();if(elements.inventoryWarehouse.value)params.set('bodegaId',elements.inventoryWarehouse.value);if(elements.inventorySearch.value.trim())params.set('q',elements.inventorySearch.value.trim());if(elements.inventoryDateFrom.value)params.set('desde',elements.inventoryDateFrom.value);if(elements.inventoryDateTo.value)params.set('hasta',elements.inventoryDateTo.value);state.warehouseReceipts=await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/warehouse-receipts-history?${params}`);renderWarehouseReceiptHistory(state.warehouseReceipts);if(!state.warehouseReceiptDetail)elements.inventoryOperationPanel.replaceChildren(emptyMessage('Selecciona una recepción para ver el informe físico y sus novedades.'));}catch(error){state.warehouseReceipts=[];state.warehouseReceiptDetail=null;elements.inventoryTable.replaceChildren(emptyMessage(error.message));elements.inventoryOperationPanel.replaceChildren();elements.inventoryStatus.textContent='No fue posible consultar';renderWarehouseHistoryStats([]);}}
function warehouseIssueStatusLabel(status){return status==='NO_RECIBIDA'?'No recibida':'Recibida con novedad';}
function renderWarehouseIssueStats(rows){
  const invoices=new Set(rows.map(x=>x.documentoProveedorId));
  const received=rows.filter(x=>x.estadoFisico==='RECIBIDA_NOVEDAD').length;
  const missing=rows.filter(x=>x.estadoFisico==='NO_RECIBIDA').length;
  const cards=[['Facturas con pendientes',invoices.size],['Novedades pendientes',rows.length],['Recibidas con novedad',received],['No recibidas',missing]];
  elements.inventoryStats.replaceChildren();
  cards.forEach(([label,value])=>{const card=document.createElement('article');const strong=document.createElement('strong');strong.textContent=value;const span=document.createElement('span');span.textContent=label;card.append(strong,span);elements.inventoryStats.append(card);});
}
function issueIdentifierText(issue){return [issue.serial&&`Serial ${issue.serial}`,issue.motor&&`Motor ${issue.motor}`,issue.chasis&&`Chasis ${issue.chasis}`,issue.vin&&`VIN ${issue.vin}`].filter(Boolean).join(' · ')||'Sin identificadores';}
function renderWarehouseReceiptIssues(rows){
  renderWarehouseIssueStats(rows);elements.inventoryTable.replaceChildren();elements.inventoryOperationPanel.replaceChildren();elements.inventoryOperationPanel.hidden=false;
  elements.inventoryStatus.textContent=`${rows.length} novedad(es) pendiente(s)`;
  if(!rows.length){elements.inventoryOperationPanel.append(emptyMessage('No hay novedades pendientes para los filtros seleccionados.'));return;}
  const groups=new Map();rows.forEach(issue=>{const key=String(issue.documentoProveedorId);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(issue);});
  const heading=document.createElement('div');heading.className='operation-heading';heading.innerHTML='<span>ADMINISTRACIÓN · BODEGA</span><h2>Bandeja de novedades pendientes</h2><p>Solo aparecen unidades recibidas con novedad o no recibidas. Al completar la gestión dejan esta bandeja y conservan su auditoría.</p>';elements.inventoryOperationPanel.append(heading);
  const list=document.createElement('div');list.className='warehouse-issue-groups';
  groups.forEach(issues=>{
    const first=issues[0];const card=document.createElement('article');card.className='warehouse-issue-card';
    const cardHead=document.createElement('header');cardHead.className='warehouse-issue-card-head';
    const title=document.createElement('div');const kicker=document.createElement('span');kicker.textContent=`${first.tipoDocumento} · ${first.recepcionNumero}`;const h3=document.createElement('h3');h3.textContent=first.numeroDocumento;const meta=document.createElement('p');meta.textContent=`${first.proveedor} · ${first.bodega} · ${first.fechaDocumento}`;title.append(kicker,h3,meta);
    const count=document.createElement('strong');count.textContent=`${issues.length} pendiente${issues.length===1?'':'s'}`;cardHead.append(title,count);card.append(cardHead);
    const unitList=document.createElement('div');unitList.className='warehouse-issue-list';
    issues.forEach(issue=>{
      const row=document.createElement('section');row.className='warehouse-issue-row';row.dataset.issueId=issue.recepcionMercanciaRevisionUnidadId;
      const detail=document.createElement('div');detail.className='warehouse-issue-detail';
      const status=document.createElement('span');status.className=`warehouse-issue-badge ${issue.estadoFisico==='NO_RECIBIDA'?'missing':'received'}`;status.textContent=warehouseIssueStatusLabel(issue.estadoFisico);
      const h4=document.createElement('h4');h4.textContent=`${issue.codigoArticulo} · ${issue.descripcion}`;
      const ids=document.createElement('p');ids.textContent=`Línea ${issue.numeroLinea} · Unidad ${issue.numeroUnidad} · ${issueIdentifierText(issue)}`;
      const note=document.createElement('blockquote');note.textContent=issue.observacion||'El auxiliar no registró una observación.';
      const reviewed=document.createElement('small');reviewed.textContent=`Reportada por ${issue.revisadoPor} · ${new Date(issue.revisadoEnUtc).toLocaleString('es-CO')}`;
      detail.append(status,h4,ids,note,reviewed);
      const form=document.createElement('form');form.className='warehouse-issue-form';
      const resultLabel=document.createElement('label');resultLabel.innerHTML='<span>Resultado de la gestión</span>';const result=document.createElement('select');result.required=true;result.append(new Option('Seleccionar…',''),new Option('Reclamo al proveedor','RECLAMO_PROVEEDOR'),new Option('Ajuste de inventario','AJUSTE_INVENTARIO'),new Option('Devolución','DEVOLUCION'),new Option('Aceptada y documentada','ACEPTADA_DOCUMENTADA'),new Option('Otra gestión','OTRA'));resultLabel.append(result);
      const noteLabel=document.createElement('label');noteLabel.innerHTML='<span>Nota administrativa</span>';const managementNote=document.createElement('textarea');managementNote.maxLength=1000;managementNote.rows=3;managementNote.placeholder='Describe la acción, responsable externo o soporte relacionado.';noteLabel.append(managementNote);
      const action=document.createElement('button');action.type='submit';action.className='button primary';action.textContent='Marcar como gestionada';form.append(resultLabel,noteLabel,action);
      form.addEventListener('submit',async event=>{event.preventDefault();if(!result.value)return;if(result.value==='OTRA'&&!managementNote.value.trim()){showError('Describe la gestión realizada cuando seleccionas Otra.');managementNote.focus();return;}try{action.disabled=true;action.textContent='Guardando gestión…';if(state.runtimeMode==='api'&&state.erpSession?.api){await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/warehouse-receipt-issues/${issue.recepcionMercanciaRevisionUnidadId}/resolve`,{method:'POST',body:JSON.stringify({resultado:result.value,observacionGestion:managementNote.value.trim()||null})});}else{state.warehouseIssues=state.warehouseIssues.filter(x=>x.recepcionMercanciaRevisionUnidadId!==issue.recepcionMercanciaRevisionUnidadId);}await refreshWarehouseReceiptIssues();showSuccess(`Novedad de la factura ${issue.numeroDocumento} gestionada.`);}catch(error){showError(error.message);action.disabled=false;action.textContent='Marcar como gestionada';}});
      row.append(detail,form);unitList.append(row);
    });
    card.append(unitList);list.append(card);
  });
  elements.inventoryOperationPanel.append(list);
}
function filterWarehouseIssueDemo(rows){
  const query=elements.inventorySearch.value.trim().toLocaleLowerCase('es-CO');const warehouse=elements.inventoryWarehouse.selectedOptions[0]?.textContent.split(' · ').pop();const type=elements.inventoryIssueType.value;const from=elements.inventoryDateFrom.value;const to=elements.inventoryDateTo.value;
  return rows.filter(x=>(!query||Object.values(x).join(' ').toLocaleLowerCase('es-CO').includes(query))&&(!elements.inventoryWarehouse.value||x.bodega===warehouse)&&(!type||x.estadoFisico===type)&&(!from||x.fechaContable>=from)&&(!to||x.fechaContable<=to));
}
async function refreshWarehouseReceiptIssues(){
  elements.inventoryStatus.textContent='Consultando novedades…';elements.inventoryNotice.hidden=true;
  try{
    if(!canUseWarehouseHistory())throw new Error('La gestión de novedades está disponible solo para administradores.');
    if(state.runtimeMode==='api'&&state.erpSession?.api){const params=new URLSearchParams();if(elements.inventoryWarehouse.value)params.set('bodegaId',elements.inventoryWarehouse.value);if(elements.inventorySearch.value.trim())params.set('q',elements.inventorySearch.value.trim());if(elements.inventoryIssueType.value)params.set('tipo',elements.inventoryIssueType.value);if(elements.inventoryDateFrom.value)params.set('desde',elements.inventoryDateFrom.value);if(elements.inventoryDateTo.value)params.set('hasta',elements.inventoryDateTo.value);state.warehouseIssues=await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/warehouse-receipt-issues?${params}`);renderWarehouseReceiptIssues(state.warehouseIssues);}
    else{if(!state.warehouseIssuesDemoInitialized){state.warehouseIssues=warehouseIssueDemo.map(x=>({...x}));state.warehouseIssuesDemoInitialized=true;}elements.inventoryNotice.textContent='Vista demostrativa local. En modo API ERP las gestiones quedan auditadas en SQL Server.';elements.inventoryNotice.hidden=false;renderWarehouseReceiptIssues(filterWarehouseIssueDemo(state.warehouseIssues));}
  }catch(error){state.warehouseIssues=[];elements.inventoryOperationPanel.replaceChildren(emptyMessage(error.message));elements.inventoryStatus.textContent='No fue posible consultar';renderWarehouseIssueStats([]);}
}
async function openWarehouseReceiptHistory(receiptId){elements.inventoryOperationPanel.hidden=false;elements.inventoryOperationPanel.replaceChildren(emptyMessage('Cargando informe de recepción…'));try{state.warehouseReceiptDetail=await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/warehouse-receipts-history/${receiptId}`);renderWarehouseReceiptHistoryDetail();elements.inventoryOperationPanel.scrollIntoView({behavior:'smooth',block:'start'});}catch(error){elements.inventoryOperationPanel.replaceChildren(emptyMessage(error.message));}}
function renderWarehouseReceiptHistoryDetail(){const detail=state.warehouseReceiptDetail;if(!detail)return;const receipt=detail.recepcion;const panel=elements.inventoryOperationPanel;panel.replaceChildren();const heading=document.createElement('div');heading.className='operation-heading';heading.innerHTML=`<span>INFORME DE RECEPCIÓN</span><h2>${receipt.numero}</h2><p>${receipt.tipoDocumento} ${receipt.numeroDocumento} · ${receipt.proveedor} · ${receipt.bodega}</p>`;panel.append(heading);const status=document.createElement('div');status.className='workflow-status';status.innerHTML=`<span>Estado<strong>${receipt.estado}</strong></span><span>Motos<strong>${receipt.unidadesSerializadas}</strong></span><span>Revisadas<strong>${receipt.revisadas}</strong></span><span>Novedad<strong>${receipt.recibidasConNovedad}</strong></span><span>No recibida<strong>${receipt.noRecibidas}</strong></span>`;panel.append(status);const table=document.createElement('table');const head=document.createElement('thead');head.innerHTML='<tr><th>Moto</th><th>Identificadores</th><th>Color / modelo</th><th>Estado físico</th><th>Observación</th><th>Revisado por</th><th>Fecha revisión</th></tr>';const body=document.createElement('tbody');detail.unidades.forEach(unit=>{const row=document.createElement('tr');const moto=document.createElement('td');moto.innerHTML=`<strong>${unit.codigoArticulo}</strong><small>Línea ${unit.numeroLinea} · Moto ${unit.numeroUnidad}</small>`;const ids=document.createElement('td');ids.textContent=[unit.serial&&`Serial ${unit.serial}`,unit.motor&&`Motor ${unit.motor}`,unit.chasis&&`Chasis ${unit.chasis}`,unit.vin&&`VIN ${unit.vin}`].filter(Boolean).join(' · ')||'Sin identificadores';const color=document.createElement('td');color.textContent=[unit.color,unit.modelo].filter(Boolean).join(' · ')||'—';const stateCell=document.createElement('td');stateCell.textContent=unit.estadoFisico||'Sin revisar';const note=document.createElement('td');note.textContent=unit.observacion||'—';const user=document.createElement('td');user.textContent=unit.revisadoPor||'—';const date=document.createElement('td');date.textContent=unit.revisadoEnUtc?new Date(unit.revisadoEnUtc).toLocaleString('es-CO'):'—';row.append(moto,ids,color,stateCell,note,user,date);body.append(row);});if(!detail.unidades.length){const row=document.createElement('tr');const cell=document.createElement('td');cell.colSpan=7;cell.className='empty';cell.textContent='Esta recepción no tiene unidades serializadas en el informe.';row.append(cell);body.append(row);}table.append(head,body);const wrap=document.createElement('div');wrap.className='table-wrap receiving-check-form';wrap.append(table);panel.append(wrap);}
...
function warehousePhysicalReviewSaved(detail=state.warehouseReceiptDetail){const receipt=detail?.recepcion;return Boolean(receipt&&(receipt.unidadesSerializadas===0||receipt.revisadas>0));}
function syncWarehouseReceiptWorkflowActions(){const reviewed=warehousePhysicalReviewSaved();const panel=elements.inventoryOperationPanel;const actions=panel.querySelector('.saved-detail-actions');if(!actions)return;const buttons=[...actions.querySelectorAll('button')];const save=buttons.find(button=>button.textContent.includes('revisión física')||button.textContent.includes('Recepción física'));const post=buttons.find(button=>button.textContent.includes('Contabilizar'));if(save){save.disabled=reviewed;if(reviewed)save.textContent='Recepción física guardada';}if(post){post.disabled=!reviewed;post.title=reviewed?'':'Primero guarda la recepción física.';}if(reviewed)panel.querySelectorAll('.receiving-check-form select,.receiving-check-form input').forEach(input=>input.disabled=true);}
const renderWarehouseReceiptDetailBase=renderWarehouseReceiptDetail;
renderWarehouseReceiptDetail=function(){renderWarehouseReceiptDetailBase();syncWarehouseReceiptWorkflowActions();};
async function refreshInventory(){const view=state.inventoryView;if(view==='operations'){renderInventoryOperationPanel();return;}if(view==='receiving'){await refreshWarehouseReceiving();return;}if(view==='receipt-issues'){await refreshWarehouseReceiptIssues();return;}if(view==='receipt-history'){await refreshWarehouseReceiptHistory();return;}elements.inventoryStatus.textContent='Actualizando…';elements.inventoryNotice.hidden=true;try{let rows;if(state.runtimeMode==='api'&&state.erpSession?.api){const base=`/api/v1/companies/${state.erpSession.company.id}`;const params=new URLSearchParams();if(elements.inventoryWarehouse.value)params.set('bodegaId',elements.inventoryWarehouse.value);if(elements.inventorySearch.value.trim())params.set('q',elements.inventorySearch.value.trim());if(view==='kardex'){if(elements.inventoryDateFrom.value)params.set('desde',elements.inventoryDateFrom.value);if(elements.inventoryDateTo.value)params.set('hasta',elements.inventoryDateTo.value);}if(view==='expiry')params.set('dias','365');const endpoint={stock:'inventory/stock',kardex:'inventory/kardex',serials:'inventory/serialized-units',expiry:'inventory/expiry-alerts'}[view];rows=await apiRequest(`${base}/${endpoint}?${params}`);}else{const query=elements.inventorySearch.value.trim().toLocaleLowerCase('es-CO');rows=inventoryDemo[view].filter(x=>!query||Object.values(x).join(' ').toLocaleLowerCase('es-CO').includes(query));elements.inventoryNotice.textContent='Vista demostrativa local. Cambia el origen de datos a API ERP para consultar y registrar movimientos reales en SQL Server.';elements.inventoryNotice.hidden=false;}state.inventoryData=rows;const table=inventoryRows(view,rows);elements.inventoryTable.replaceChildren(buildDataTable(table.headers,table.rows));renderInventoryStats(rows);elements.inventoryStatus.textContent=`${rows.length} registros consultados`;}catch(error){elements.inventoryTable.replaceChildren(emptyMessage(error.message));elements.inventoryStatus.textContent='No fue posible consultar';}}
function showInventoryView(view='stock'){if(!isInventoryViewAllowed(view)){const fallback=firstAllowedInventoryView();if(fallback&&fallback!==view){showInventoryView(fallback);return;}routeToDefaultWorkspace(true);return;}state.inventoryView=view;elements.purchaseModule.hidden=true;elements.masterDataModule.hidden=true;elements.savedPurchasesModule.hidden=true;elements.accountsPayableModule.hidden=true;elements.inventoryModule.hidden=false;elements.advancedControlsModule.hidden=true;elements.securityModule.hidden=true;elements.savedPurchasesNav.classList.remove('active');elements.accountsPayableNav.classList.remove('active');elements.controlsNav.classList.remove('active');elements.securityAdminNav.classList.remove('active');document.querySelector('.breadcrumb span').textContent=view==='receiving'?'Bodega':view==='receipt-history'||view==='receipt-issues'?'Administración':'Inventario';elements.breadcrumbCurrent.textContent={stock:'Existencias',receiving:'Recepción física','receipt-issues':'Novedades pendientes','receipt-history':'Historial recepciones',kardex:'Kardex',serials:'Seriales',expiry:'Vencimientos',operations:'Movimientos'}[view];document.querySelectorAll('[data-registration-mode],[data-master-view]').forEach(x=>x.classList.remove('active'));document.querySelectorAll('[data-inventory-view]').forEach(x=>x.classList.toggle('active',x.dataset.inventoryView===view));const isKardex=view==='kardex';const isOperations=view==='operations';const isReceiving=view==='receiving';const isReceiptHistory=view==='receipt-history';const isReceiptIssues=view==='receipt-issues';elements.inventoryDateFromField.hidden=!(isKardex||isReceiptHistory||isReceiptIssues);elements.inventoryDateToField.hidden=!(isKardex||isReceiptHistory||isReceiptIssues);elements.inventoryIssueTypeField.hidden=!isReceiptIssues;elements.inventoryTable.hidden=isOperations||isReceiptIssues;elements.inventoryOperationPanel.hidden=!(isOperations||isReceiving||isReceiptHistory||isReceiptIssues);elements.inventorySearch.closest('label').hidden=isOperations;elements.inventoryWarehouse.closest('label').hidden=isOperations;$('#refreshInventory').hidden=isOperations;populateInventoryWarehouses();if(isReceiving||isReceiptHistory)state.warehouseReceiptDetail=null;void refreshInventory();window.scrollTo({top:0,behavior:'smooth'});}
...
function accountingToday() {
  return new Intl.DateTimeFormat('sv-SE',{timeZone:state.erpSession?.company?.zonaHoraria||'America/Bogota',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
}
function receiptPeriodForDate(date,periods=state.apiContext?.periods||[]) {
  const matches=periods.filter(p=>['ABIERTO','REABIERTO'].includes(p.estado)&&date>=p.fechaInicio.slice(0,10)&&date<=p.fechaFin.slice(0,10));
  return matches.length===1?matches[0]:null;
}
function bindReceiptAccountingDate(date,period,container,onChange=()=>{}) {
  const notice=document.createElement('p');notice.className='workflow-notice';container.append(notice);
  const sync=()=>{
    const match=receiptPeriodForDate(date.value);
    period.value=match?String(match.periodoInventarioId):'';
    date.setCustomValidity(match?'':'No hay un período abierto para esta fecha contable.');
    notice.textContent=match?`Período automático: ${match.codigo} · ${match.estado}`:'No hay un período abierto para esta fecha contable. Abre el mes en Inventarios → Períodos de inventario.';
    onChange();
  };
  date.addEventListener('change',sync);sync();return sync;
}
...
function buildPurchaseWorkflowPanel(invoice) {
  ...
  if(inventoryLines.length&&!state.apiContext.warehouses.length){
  ...
  const date=document.createElement('input'); date.type='date'; date.value=invoice.accountingDate||accountingToday();date.addEventListener('change',()=>{invoice.accountingDate=date.value;});
  const invoiceDate=document.createElement('input');invoiceDate.type='date';invoiceDate.value=invoice.issueDate||'';invoiceDate.readOnly=true;
  ...
  const warehouseControl=controlField('Bodega',warehouse);const periodControl=controlField('Periodo de inventario',period);periodControl.hidden=true;const dateControl=controlField('Fecha contable',date);const paymentControl=controlField('Condición de pago',payment);const creditControl=controlField('Días de crédito',creditDays);
  warehouseControl.hidden=inventoryLines.length===0;
  controls.append(warehouseControl,controlField('Fecha de factura',invoiceDate),dateControl,paymentControl,creditControl,autoCreateLabel); panel.append(controls);
  ...
  if(inventoryLines.length)bindReceiptAccountingDate(date,period,panel,updateCompleteState);
...
function setRegistrationMode(mode) {
  ...
  elements.accountsPayableNav.classList.remove('active');
  document.querySelector('.breadcrumb span').textContent = 'Compras';
  document.querySelectorAll('[data-master-view],[data-inventory-view]').forEach((button) => button.classList.remove('active'));
...
async function openCompanyManager() {
  if(!state.erpSession?.superAdmin&&!state.pendingSuperAdmin)return;
...
async function beginLogin(event) {
  ...
  try {
    ...
    renderCompanyOptions(companies);configureSuperAdminCompanyPanel(state.pendingSuperAdmin,companies.length>0);
    if(state.pendingSuperAdmin)openErpDialog(elements.companyDialog);
    else {
      if(companies.length!==1||String(companies[0].empresaId)!==String(login.empresaId))throw new Error('Tu usuario debe tener una única empresa asignada. Contacta al superadministrador.');
      selectCompany($('#companyOptions').firstElementChild);
    }
...
async function saveManualDraft(event) {
  ...
  if(isApiManual){
    ...
    const accountingPeriod=receiptPeriodForDate($('#manualAccountingDate').value);if(needsWarehouse&&!accountingPeriod)return showError('Abre el período de la fecha contable en Inventarios → Períodos de inventario.');if(accountingPeriod)elements.manualPeriod.value=String(accountingPeriod.periodoInventarioId);
  ...
  if(isApiManual){
    ...
    try{
      const preparation={hasInventory:needsWarehouse,hasServices:lines.some(x=>x.classification==='service'),bodegaId:draft.warehouse,periodoInventarioId:draft.period,fechaContable:draft.accountingDate||accountingToday(),numeroDocumento:draft.invoiceNumber};
      ...
      if(intent==='post'&&receiptId&&current.recepcionEstado!=='CONTABILIZADA'){
        await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/receipts/${receiptId}/post`,{method:'POST',body:JSON.stringify({fechaContable:draft.accountingDate||accountingToday(),correlationId:crypto.randomUUID?crypto.randomUUID():null})});
      }
...
$('#manualAccountingDate').value=accountingToday();
...
elements.accountsPayableNav.addEventListener('click',showAccountsPayable);
elements.refreshSavedPurchases.addEventListener('click',()=>void refreshSavedPurchases());
elements.refreshAccountsPayable.addEventListener('click',()=>void refreshAccountsPayable());
elements.accountsPayableSupplier.addEventListener('change',()=>void refreshAccountsPayable());
elements.accountsPayableState.addEventListener('change',()=>void refreshAccountsPayable());
elements.accountsPayableTable.addEventListener('click',event=>{const button=event.target.closest('[data-payable-document-id]');if(button){showSavedPurchases();void openSavedPurchase(Number(button.dataset.payableDocumentId));}});
...
elements.inventoryNav.addEventListener('click',()=>{if(!elements.inventoryNav.dataset.inventoryView)showInventoryView(isInventoryViewAllowed(state.inventoryView)?state.inventoryView:firstAllowedInventoryView()||'receiving');});
...
elements.inventoryIssueType.addEventListener('change',()=>void refreshInventory());
elements.inventoryDateFrom.addEventListener('change',()=>void refreshInventory());
elements.inventoryDateTo.addEventListener('change',()=>void refreshInventory());
elements.inventoryTable.addEventListener('click',event=>{const historyButton=event.target.closest('[data-warehouse-history-id]');if(historyButton){void openWarehouseReceiptHistory(Number(historyButton.dataset.warehouseHistoryId));return;}const button=event.target.closest('[data-warehouse-receipt-id]');if(button)void openWarehouseReceipt(Number(button.dataset.warehouseReceiptId));});

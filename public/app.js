...
function isInventoryViewAllowed(view) {
  ...
  if (view === 'receipt-history') return canUseWarehouseHistory();
  if (view === 'operations') return canUseInventoryOperations();
  return canUseInventoryReports();
}
function firstAllowedInventoryView() {
  return ['receiving', 'receipt-history', 'stock', 'kardex', 'serials', 'expiry', 'operations'].find(isInventoryViewAllowed) || null;
}

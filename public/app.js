...
function savedPurchaseEffectiveState(document){
  if(document.estado==='RECHAZADO')return 'ANULADA';
  if(document.recepcionEstado==='CONTABILIZADA')return 'CONTABILIZADA';
  if(document.recepcionMercanciaId)return 'PENDIENTE DE CONTABILIZAR';
  return document.estado==='VALIDADO'?'PREPARADA':document.estado;
}
...
function renderSavedPurchases(){
  ...
  state.savedPurchaseDocuments.forEach(document=>{const row=document.createElement('tr');const identity=document.createElement('td');identity.className='saved-document-identity';const strong=document.createElement('strong');strong.textContent=document.numeroDocumento;const small=document.createElement('small');small.textContent=`${document.tipoDocumento} · ${document.fuente}`;identity.append(strong,small);const supplier=document.createElement('td');supplier.textContent=`${document.proveedor} · ${document.proveedorIdentificacion}`;const date=document.createElement('td');date.textContent=document.fechaDocumento;const payment=document.createElement('td');payment.textContent=document.condicionPago==='CREDITO'?'Crédito':'Contado';const total=document.createElement('td');total.textContent=manualCurrency(document.totalPagar);const serials=document.createElement('td');serials.textContent=document.unidadesSerializadas;const status=document.createElement('td');const badge=document.createElement('span');const effective=savedPurchaseEffectiveState(document);badge.className=`saved-purchase-badge state-${effective.toLowerCase().replaceAll(' ','-')}`;badge.textContent=effective;status.append(badge);const action=document.createElement('td');const open=document.createElement('button');open.type='button';open.className='button secondary';open.dataset.savedPurchaseId=document.documentoProveedorId;open.textContent='Abrir';action.append(open);row.append(identity,supplier,date,payment,total,serials,status,action);body.append(row);});
...
function renderSavedPurchaseDetail(){
  const detail=state.savedPurchaseDetail;if(!detail)return;const document=detail.documento;const inventoryLines=detail.lineas.filter(x=>x.clasificacion==='INVENTARIO');const serviceLines=detail.lineas.filter(x=>x.clasificacion==='SERVICIO_GASTO');const panel=elements.savedPurchaseDetail;panel.replaceChildren();
  const heading=document.createElement('div');heading.className='saved-detail-heading';const copy=document.createElement('div');const kicker=document.createElement('span');kicker.textContent='DETALLE DE LA ENTRADA';const title=document.createElement('h2');title.textContent=`${document.tipoDocumento} ${document.numeroDocumento}`;const subtitle=document.createElement('p');subtitle.textContent=`${document.proveedor} · ${document.proveedorIdentificacion} · ${manualCurrency(document.totalPagar)}`;copy.append(kicker,title,subtitle);const close=document.createElement('button');close.type='button';close.className='button secondary';close.textContent='Cerrar detalle';close.addEventListener('click',()=>{panel.hidden=true;state.savedPurchaseDetail=null;});heading.append(copy,close);panel.append(heading);
  const status=document.createElement('div');status.className='workflow-status';status.innerHTML=`<span>Documento<strong>${document.estado}</strong></span><span>Recepción<strong>${document.recepcionEstado||'Sin preparar'}</strong></span><span>Pago<strong>${document.condicionPago==='CREDITO'?`Crédito · ${document.diasCredito} días`:'Contado'}</strong></span><span>Seriales<strong>${document.unidadesSerializadas}</strong></span><span>XML<strong>${document.xmlOriginalGuardado?'Guardado':'No aplica'}</strong></span>`;panel.append(status);
  ...
  if(document.estado==='RECHAZADO'||document.recepcionEstado==='CONTABILIZADA'){const complete=document.createElement('p');complete.className='saved-detail-result';complete.textContent=document.estado==='RECHAZADO'?'Este borrador fue anulado y se conserva únicamente para auditoría.':`Entrada contabilizada${document.recepcionNumero?` como ${document.recepcionNumero}`:''}. Las motos ya están disponibles en Inventario → Operación diaria → Seriales.`;panel.append(complete);return;}
  const actions=document.createElement('div');actions.className='saved-detail-actions';
  if(inventoryLines.length){
    const controls=document.createElement('div');controls.className='workflow-controls';const warehouse=document.createElement('select');warehouse.append(new Option('Selecciona bodega…',''));(state.apiContext?.warehouses||[]).forEach(x=>warehouse.add(new Option(`${x.codigo} · ${x.nombre}`,x.bodegaId)));const period=document.createElement('select');period.append(new Option('Selecciona periodo…',''));(state.apiContext?.periods||[]).forEach(x=>period.add(new Option(`${x.codigo} · ${x.estado}`,x.periodoInventarioId)));const date=document.createElement('input');date.type='date';date.value=document.fechaDocumento;const field=(text,input)=>{const label=document.createElement('label');label.className='workflow-field';const span=document.createElement('span');span.textContent=text;label.append(span,input);return label;};controls.append(field('Bodega',warehouse),field('Periodo de inventario',period),field('Fecha contable',date));controls.hidden=Boolean(document.recepcionMercanciaId);panel.append(controls);
    const post=document.createElement('button');post.type='button';post.className='button primary large';post.textContent=document.recepcionMercanciaId?'Contabilizar entrada':'Preparar y contabilizar entrada';post.disabled=!document.recepcionMercanciaId&&(!warehouse.value||!period.value);const validate=()=>post.disabled=!warehouse.value||!period.value;warehouse.addEventListener('change',validate);period.addEventListener('change',validate);
    post.addEventListener('click',async()=>{if(!window.confirm(`Se afectarán las existencias y el Kardex con la factura ${document.numeroDocumento}. ¿Deseas continuar?`))return;try{post.disabled=true;post.textContent='Contabilizando…';let receiptId=document.recepcionMercanciaId;if(!receiptId){const prepared=await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/supplier-documents/${document.documentoProveedorId}/prepare`,{method:'POST',body:JSON.stringify({bodegaId:Number(warehouse.value),periodoInventarioId:Number(period.value),fechaContable:date.value,numeroRecepcion:`ENT-${document.numeroDocumento}`.slice(0,50),numeroCausacion:serviceLines.length?`CAU-${document.numeroDocumento}`.slice(0,50):null})});receiptId=prepared.recepcionMercanciaId;}await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/receipts/${receiptId}/post`,{method:'POST',body:JSON.stringify({correlationId:crypto.randomUUID?crypto.randomUUID():null})});await Promise.all([refreshSavedPurchases(),loadApiCompanyContext()]);await openSavedPurchase(document.documentoProveedorId);showSuccess('Entrada contabilizada. Los seriales ya están disponibles en inventario.');}catch(error){showError(`No fue posible contabilizar la entrada. ${error.message}`);post.disabled=false;post.textContent=document.recepcionMercanciaId?'Contabilizar entrada':'Preparar y contabilizar entrada';}});actions.append(post);
  }
  if(document.estado==='BORRADOR'&&!document.recepcionMercanciaId&&!document.causacionServicioId){const reject=document.createElement('button');reject.type='button';reject.className='button danger';reject.textContent='Anular borrador';reject.addEventListener('click',async()=>{if(!window.confirm(`¿Anular el borrador ${document.numeroDocumento}?`))return;try{reject.disabled=true;await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/supplier-documents/${document.documentoProveedorId}/reject`,{method:'POST',body:'{}'});await refreshSavedPurchases();await openSavedPurchase(document.documentoProveedorId);}catch(error){showError(error.message);reject.disabled=false;}});actions.append(reject);}panel.append(actions);
...
function buildPurchaseWorkflowPanel(invoice) {
  ...
  const save=document.createElement('button'); save.type='button'; save.className='button secondary'; save.textContent=workflow?'Borrador guardado':'Guardar solo como borrador'; save.disabled=Boolean(workflow);
  ...
  const updateCompleteState=()=>{ complete.disabled=workflow?.recepcionEstado==='CONTABILIZADA'||(pending.length>0&&!autoCreate.checked)||(inventoryLines.length>0&&!workflow?.recepcionMercanciaId&&(!warehouse.value||!period.value)); };
...
async function saveManualDraft(event) {
  ...
  if(isApiManual){
    ...
    try{const result=await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/supplier-documents`,{method:'POST',body:JSON.stringify(buildManualSupplierDocumentPayload(draft))});let receiptId=null;if(intent==='post'){const current=await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/supplier-documents/${result.documentoProveedorId}`);if(!current.recepcionMercanciaId&&!current.causacionServicioId){const prepared=await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/supplier-documents/${result.documentoProveedorId}/prepare`,{method:'POST',body:JSON.stringify({bodegaId:needsWarehouse?Number(draft.warehouse):null,periodoInventarioId:needsWarehouse?Number(draft.period):null,fechaContable:draft.issueDate,numeroRecepcion:needsWarehouse?`ENT-${draft.invoiceNumber}`.slice(0,50):null,numeroCausacion:lines.some(x=>x.classification==='service')?`CAU-${draft.invoiceNumber}`.slice(0,50):null})});receiptId=prepared.recepcionMercanciaId;}else receiptId=current.recepcionMercanciaId;if(receiptId&&current.recepcionEstado!=='CONTABILIZADA')await apiRequest(`/api/v1/companies/${state.erpSession.company.id}/receipts/${receiptId}/post`,{method:'POST',body:JSON.stringify({correlationId:crypto.randomUUID?crypto.randomUUID():null})});elements.manualFormStatus.textContent=receiptId?'Entrada guardada y contabilizada':'Causación guardada y preparada';showSuccess(receiptId?'Entrada contabilizada con sus seriales.':'Causación preparada para asignación contable.');}else{elements.manualFormStatus.textContent=result.yaExistia?'Documento existente recuperado sin duplicar':'Borrador guardado en SQL Server';showSuccess('Borrador guardado. Puedes recuperarlo en Entradas guardadas.');}await refreshManualWorkflow(result.documentoProveedorId,receiptId);}

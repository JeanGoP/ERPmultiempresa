...
function zeusRenderSettings(){
  ...
  $('#zeusContent').innerHTML=`<form id="zeusSettingsForm"><section class="zeus-card"><h2>1. Destino y comprobante</h2><div class="zeus-grid">${zeusField('Servidor SQL esperado','servidorEsperado',s.servidorEsperado,'text',150)}${zeusField('Base contable de Zeus','baseEsperada',s.baseEsperada,'text',128)}${zeusField('Fuente (2 caracteres)','fuente',s.fuente,'text',2)}${zeusField('Serie (2 dígitos)','serie',s.serie,'text',2)}${zeusField('Unidad de negocio','unidadNegocio',s.unidadNegocio)}${zeusField('Usuario de Zeus','usuarioZeus',s.usuarioZeus)}${zeusField('Tipo de factura','tipoFactura',s.tipoFactura,'text',10)}</div><button type="button" data-zeus="check" class="button secondary" ${zeusUI.version?'':'disabled'}>Comprobar conexión guardada</button></section>
    ${zeusGeneralSupplierSection(s)}${zeusRetentionSection(s)}${typeof zeusSourcesSection==='function'?zeusSourcesSection(s):''}
...
zeusPanel.addEventListener('click',event=>{
  ...
  if(typeof zeusSourceClick==='function'&&zeusSourceClick(button))return;

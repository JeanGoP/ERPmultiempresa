...
function zeusSourceRow(rule={}){
  const saved=zeusUI.settings||{};
  const documentFields='<label>Unidad de negocio<input name="sourceUnit" maxlength="20" required value="'+zeusEscape(rule.unidadNegocio??saved.unidadNegocio??'')+'"></label><label>Tipo de documento<input name="sourceDocument" maxlength="10" required value="'+zeusEscape(rule.tipoFactura??saved.tipoFactura??'')+'"></label>';
  return '<div class="zeus-source-rule zeus-grid"><label>Sucursal / punto<select name="sourceBranch" required data-branch-id="'+zeusEscape(rule.sucursalId||'')+'" data-branch-name="'+zeusEscape(rule.sucursal||'')+'">'+zeusBranchOptions(rule.sucursalId,rule.sucursal||'')+'</select></label><label>Movimiento<select name="sourceMovement">'+Object.entries(zeusSourceMovements).map(([id,label])=>'<option value="'+id+'" '+(rule.movimiento===id?'selected':'')+'>'+label+'</option>').join('')+'</select></label><label>Fuente<input name="sourceCode" maxlength="2" minlength="2" required value="'+zeusEscape(rule.fuente||'')+'"></label><label>Serie<input name="sourceSeries" maxlength="2" pattern="[0-9]{2}" required value="'+zeusEscape(rule.serie||'00')+'"></label>'+documentFields+'<button type="button" class="button secondary" data-source-action="remove">Quitar</button></div>';
}
function zeusSourcesSection(settings){
  zeusSourceBranches=null;
  const unique=(settings.fuentesAutomaticas||[]).filter((r,i,all)=>all.findIndex(x=>(x.sucursalId||x.sucursal)===(r.sucursalId||r.sucursal)&&x.movimiento===r.movimiento&&x.fuente===r.fuente&&x.serie===r.serie&&x.unidadNegocio===r.unidadNegocio&&x.tipoFactura===r.tipoFactura)===i);
  const legacy=!unique.some(r=>r.movimiento==='ENTRADA_MERCANCIA')&&settings.fuente?'<details class="zeus-legacy"><summary>Configuración anterior conservada</summary><p>'+zeusEscape([settings.fuente,settings.serie,settings.unidadNegocio,settings.tipoFactura].filter(Boolean).join(' · '))+'</p></details>':'';
  return '<div class="zeus-sources"><h3>Fuentes por sucursal</h3>'+legacy+'<div id="zeusSourceRules">'+unique.map(zeusSourceRow).join('')+'</div><button type="button" class="button secondary" data-source-action="add">+ Agregar fuente</button><p id="zeusSourceStatus" role="status"></p></div>';
}
...
function zeusReadSources(settings){
  ...
  return [...document.querySelectorAll('.zeus-source-rule')].map(row=>{
    ...
    return {sucursalId:branch.id,sucursal:branch.nombre,movimiento,fuente:value('sourceCode'),serie:value('sourceSeries'),unidadNegocio:value('sourceUnit'),tipoFactura:value('sourceDocument'),usuarios:[]};
  });
}
function zeusSourceClick(button){
  if(!button.dataset.sourceAction)return false;
  if(button.dataset.sourceAction==='add')$('#zeusSourceRules').insertAdjacentHTML('beforeend',zeusSourceRow({fuente:zeusUI.settings?.fuente||'',serie:zeusUI.settings?.serie||''}));else button.closest('.zeus-source-rule').remove();

...
try {
    ...
    $migration='052_warehouse_zeus_accounts'
    ...
    $command.CommandText=@"
IF NOT EXISTS(SELECT 1 FROM core.SchemaMigration WHERE MigrationId='052_warehouse_zeus_accounts') THROW 51740,'Migracion no registrada',1;
IF OBJECT_ID('core.ZeusBodegaCuenta','U') IS NULL THROW 51741,'Tabla no encontrada',1;
IF (SELECT COUNT(*) FROM sys.security_predicates WHERE target_object_id=OBJECT_ID('core.ZeusBodegaCuenta'))<>3 THROW 51741,'RLS incompleta',1;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID('core.ZeusBodegaCuenta') AND name='FK_ZeusBodegaCuenta_Bodega' AND is_disabled=0 AND is_not_trusted=0) THROW 51741,'FK de bodega no verificada',1;
SELECT name FROM sys.columns WHERE object_id=OBJECT_ID('core.ZeusBodegaCuenta');
"@
    $verified=New-Object System.Data.DataTable;$verified.Load($command.ExecuteReader())
    $verified | Format-Table -Wrap
    Write-Output 'Verificado en remoto: migracion 052, tabla, FK y tres predicados RLS. No se modificaron registros en Zeus.'
